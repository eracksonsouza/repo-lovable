...
interface FinanceContextType {
  ...
  addInstallment: (
    installment: Omit<
      Installment,
      "id" | "createdAt" | "paidInstallments" | "monthlyAmount"
    >
  ) => Promise<void>;
...
export const FinanceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [allInstallments, setAllInstallments] = useState<
    Array<Installment & { month: string }>
  >([]);
  ...
  const currentMonthIncomes = useMemo(() => {
    return incomes.filter(
      (income) => getMonthKeyFromDate(income.date) === selectedMonth
    );
  }, [incomes, selectedMonth]);

  const currentMonthExpenses = useMemo(() => {
    return expenses.filter(
      (expense) => getMonthKeyFromDate(expense.date) === selectedMonth
    );
  ...
  const currentMonthInstallments = useMemo(() => {
    return installments.filter(
      (installment) =>
        getMonthKeyFromDate(installment.startDate) === selectedMonth
    );
  ...
  const availableMonths = useMemo(() => {
    ...
    expenses.forEach((expense) =>
      months.add(getMonthKeyFromDate(expense.date))
    );
    installments.forEach((installment) =>
      months.add(getMonthKeyFromDate(installment.startDate))
    );
  ...
  const addInstallment = useCallback(
    async (
      ...
      const monthlyAmount = Number(
        (installment.totalAmount / installment.installments).toFixed(2)
      );
      const category = categories.find(
        (cat) => cat.name === installment.category
      );
      ...
      for (let index = 0; index < installment.installments; index++) {
        ...
        expensePromises.push(
          addExpense({
            ...
            description: `${installment.name} (${index + 1}/${
  ...
  const getMonthlyTotals = useCallback(
    (month: string) => {
      const monthIncomes = incomes.filter(
        (income) => getMonthKeyFromDate(income.date) === month
      );
      const monthExpenses = expenses.filter(
        (expense) => getMonthKeyFromDate(expense.date) === month
      );
  ...
  const getMonthSnapshot = useCallback(
    (month: string) => {
      ...
      const monthInstallments = installments.filter(
        (installment) => getMonthKeyFromDate(installment.startDate) === month
      );
